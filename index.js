				...
				files.forEach(function(file) {
					try {
						...
						if (options.create_source_map !== false) {
							...
							if (process.platform == 'win32') {
								var sourceMapOutputDump = temp.openSync('gcc-dump-', 'w+').path;
							} else {
								var sourceMapOutputDump = path.join(os.tmpdir(), 'gcc-' + file + '.dump');
						...
						jvm.spawnJar(gcc, gccProcessArgs, function(err, proc) {
							if (err) {
								...
								.on('end', function() {
									...
									if (processedFiles.length == (numberOfFilesToProcess - 1) || numberOfFilesToProcess == 0) {
										resolve();
									} else {
